...
\end{lemma}
\begin{proof} Let $f_1(x)=f(x_1,\cdots,x_{N-1},|x_N|),$ $x\in B(z,2\epsilon).$ It is immediate {\color{red} we, probably, need reference} that $f_1\in\dot{W}^{\frac{N}{p},p}(B(z,2\epsilon)).$ In other words,
...
\end{lemma}
\begin{proof} Set $r(x)=(x_1,\cdots,x_{N-1},-x_N),$ $x\in B(z,\epsilon)$ and let $f_1=f+f\circ r,$ $f_2=f-f\circ r.$ It is known that {\color{red} provide reference} that $f_1\in \dot{W}^{\frac{N}{p},p}(B(z,\epsilon)).$ Hence, we may assume without loss of generality that $f=-f\circ r$ and
$$\int_{B(z,\epsilon)}\frac{|f|^p(x)dx}{|x_N|^N}<\infty.$$

We have
$$\int_{B(z,\epsilon)\times B(z,\epsilon)}\frac{|f(x)-f(y)|^pdxdy}{|x-y|^{2N}}\leq$$
$$\leq\int_{\substack{x\in H_+\cap B(z,\epsilon)\\ y\in H_+\cap B(z,\epsilon)}}\frac{|f(x)-f(y)|^pdxdy}{|x-y|^{2N}}+\int_{\substack{x\in H_-\cap B(z,\epsilon)\\ y\in H_-\cap B(z,\epsilon)}}\frac{|f(x)-f(y)|^pdxdy}{|x-y|^{2N}}+$$
$$+2^{p-1}\int_{\substack{x\in H_+\cap B(z,\epsilon)\\ y\in H_-\cap B(z,\epsilon)}}\frac{|f|^p(x)dxdy}{|x-y|^{2N}}+2^{p-1}\int_{\substack{x\in H_+\cap B(z,\epsilon)\\ y\in H_-\cap B(z,\epsilon)}}\frac{|f|^p(y)dxdy}{|x-y|^{2N}}+$$
$$+2^{p-1}\int_{\substack{x\in H_-\cap B(z,\epsilon)\\ y\in H_+\cap B(z,\epsilon)}}\frac{|f(x)|^pdxdy}{|x-y|^{2N}}+2^{p-1}\int_{\substack{x\in H_-\cap B(z,\epsilon)\\ y\in H_+\cap B(z,\epsilon)}}\frac{|f|^p(y)dxdy}{|x-y|^{2N}}.$$

It remains to note that, for every $x\in H_+,$
$$\int_{y\in H_-\cap B(z,\epsilon)}\frac{dy}{|x-y|^{2N}}\leq \int_{y\in\mathbb{R}^N}\frac{dy}{(|y|'^2+(|y_N|+|x_N|)^2)^N}=c_N|x_N|^{-N}.$$
Thus,
$$\int_{B(z,\epsilon)\times B(z,\epsilon)}\frac{|f(x)-f(y)|^pdxdy}{|x-y|^{2N}}\leq$$
$$\leq\int_{\substack{x\in H_+\cap B(z,\epsilon)\\ y\in H_+\cap B(z,\epsilon)}}\frac{|f(x)-f(y)|^pdxdy}{|x-y|^{2N}}+\int_{\substack{x\in H_-\cap B(z,\epsilon)\\ y\in H_-\cap B(z,\epsilon)}}\frac{|f(x)-f(y)|^pdxdy}{|x-y|^{2N}}+$$
$$+2^{p-1}c_N\int_{x\in H_+\cap B(z,\epsilon)}\frac{|f|^p(x)dx}{|x_N|^N}+2^{p-1}c_N\int_{H_-\cap B(z,\epsilon)}\frac{|f|^p(y)dy}{|y_N|^N}+$$
$$+2^{p-1}c_N\int_{H_-\cap B(z,\epsilon)}\frac{|f(x)|^pdx}{|x_N|^N}+2^{p-1}c_N\int_{H_+\cap B(z,\epsilon)}\frac{|f|^p(y)dy}{|y_N|^N}.$$
...
Also,
$$w(B(x,|x-y|))\leq c_{z,\epsilon,k}|x-y|^N\cdot (|x-y|+|x_N|)^K,\quad x,y\in B(z,2\epsilon).$$
Consequently,
$$w(B(x,|x-y|))\leq c_{z,\epsilon,k}|x-y|^{N+K},\quad x\in H_+\cap B(z,2\epsilon),\quad y\in H_-\cap B(z,2\epsilon).$$

We, therefore, have
$$\int_{\substack{x\in H_+\cap B(z,2\epsilon)\\ y\in H_-\cap B(z,2\epsilon)}}\frac{|f(x)-f(y)|^p}{|x-y|^{2N+2K}}|x_N|^K|y_N|^Kdxdy<\infty.$$
On the other hand, $H_+\cap B(z,2\epsilon)\subset\Gamma_1$ and $H_-\cap B(z,2\epsilon)\subset\Gamma_2.$ Thus, $f\in\dot{W}^{\frac{N}{p},p}(H_+\cap B(z,2\epsilon))$ and $f\in\dot{W}^{\frac{N}{p},p}(H_-\cap B(z,2\epsilon)).$ By Lemma \ref{s9 compute lemma} and Lemma \ref{standard gluing lemma}, $f\in\dot{W}^{\frac{N}{p},p}(B(z,\epsilon)).$
\end{proof}
...
\section{Global result from Hytonen}
\begin{lemma}\label{lem:standard-Besov-dw-dx}
Let \(1<p<\infty\). Then, for every measurable function
\(b\) on \(\mathbb R^N\),
\[
  \|b\|_{B^p(\mathbb R^N,dw)}
  \sim
  \|b\|_{B^p(\mathbb R^N,dx)},
\]
where
\[
\|b\|_{B^p(\mathbb R^N,d\mu)}
:=
\left(
...
Write
\[
  h_k(x):=\prod_{v\in R}|\langle x,v\rangle|^{k(v)},
  \qquad dw(x)=h_k(x)\,dx .
\]
\(dx\) is obvious doubling, and \(dw\) is doubling
by \eqref{doublinginequality}.

We first verify that \(dx\) and \(dw\) satisfy the \(A_\infty\)-relation used in
\cite[Proposition 1.2]{Hytonen-new}. Fix \(s>1\), and set
\[
  dw_s(x):=h_k(x)^s\,dx
  =
  \prod_{v\in R}|\langle x,v\rangle|^{s k(v)}\,dx .
\]
We have
\[
  w_s(B(x,r))
  \sim
  r^N
  \prod_{v\in R}\bigl(|\langle x,v\rangle|+r\bigr)^{s k(v)} .
\]
On the other hand, \eqref{twosideinequ} gives
\[
  w(B(x,r))
  \sim
  r^N
  \prod_{v\in R}\bigl(|\langle x,v\rangle|+r\bigr)^{k(v)} .
\]
Since \(|B(x,r)|\sim r^N\), we obtain
\begin{align*}
  \left(\fint_{B(x,r)} h_k(z)^s\,dz\right)^{1/s}
  &=
  \left(\frac{w_s(B(x,r))}{|B(x,r)|}\right)^{1/s}  \\
  &\sim
  \prod_{v\in R}\bigl(|\langle x,v\rangle|+r\bigr)^{k(v)}        \\
  &\sim
  \frac{w(B(x,r))}{|B(x,r)|}
  =
  \fint_{B(x,r)} h_k(z)\,dz .
\end{align*}
Thus \(h_k\in RH_s(dx)\). Hence, by
\cite[Proposition 3.6]{Hytonen-new}, the two doubling measures
\(dx\) and \(dw=h_k\,dx\) satisfy the \(A_\infty\)-relation
\[
  |E|\leq \varepsilon |B|
  \quad\Longrightarrow\quad
  w(E)\leq (1-\delta)w(B)
\]
for some \(\varepsilon,\delta\in(0,1)\), every Euclidean ball \(B\), and every
measurable set \(E\subset B\).

Applying \cite[Proposition 1.2]{Hytonen-new} with
\(\mu=dx\), \(\nu=dw\), and \(q=p\), we get
\[
  \|b\|_{{\rm Osc}^{p,p}(\mathbb R^N,dx)}
  \sim
  \|b\|_{{\rm Osc}^{p,p}(\mathbb R^N,dw)} .
\]
Finally, since both
\[
  (\mathbb R^N,\|\cdot\|,dx)
  ...
  (\mathbb R^N,\|\cdot\|,dw)
\]
are doubling metric measure spaces, \cite[Proposition 11.5]{HKarxiv} yields
\[
  \|b\|_{B^p(\mathbb R^N,dx)}
  \sim
  \|b\|_{{\rm Osc}^{p,p}(\mathbb R^N,dx)}
...
\[
  ...
  \|b\|_{{\rm Osc}^{p,p}(\mathbb R^N,dw)} .
\]
Combining the last three equivalences gives
\[
  \|b\|_{B^p(\mathbb R^N,dw)}
  \sim
  \|b\|_{B^p(\mathbb R^N,dx)} .
