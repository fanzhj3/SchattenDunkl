...
\begin{lemma}\label{for reverse holder first lemma} We have
$$\sup_{x\in B(x_0,r)}w(x)\leq \frac{c_k}{m(B(x_0,r))}\int_{B(x_0,r)}w(x)dx.$$
\end{lemma}
...
Fix chamber $\Gamma$ such that $x_0\in\Gamma.$ We have
$$\int_{B(x_0,r)}w(x)dx\geq\int_{x_0+(B(0,r)\cap\Gamma)}w(x)dx=\int_{B(0,r)\cap\Gamma}w(x_0+y)dy.$$
If $y\in\Gamma,$ then
...
\begin{lemma}\label{for reverse holder second lemma} Suppose $1<t<1+\frac1{|R|\cdot\max_{v\in R}k(v)}.$ For every $r>0,$ we have
$$\Big(\frac{\int_{B(x_0,r)}w^{1-t}(x)dx}{\int_{B(x_0,r)}w(x)dx}\Big)^{\frac1t}\leq c_{t,k} \frac{m(B(x_0,r))}{\int_{B(x_0,r)}w(x)dx}.$$
\end{lemma}
...
Let $B\subset\mathbb{R}^N$ be a ball. By Lemmas \ref{for reverse holder first lemma} and \ref{for reverse holder second lemma},
$$\Big(\frac1{m(B)}\int_Bw^t(x)dx\Big)^{\frac1t}\leq\frac{c_k}{m(B)}\int_{\Gamma}w(x)dx,\quad t>0.$$
$$\Big(\frac{\int_Bw^{1-t}(x)dx}{\int_Bw(x)dx}\Big)^{\frac1t}\leq c_G \frac{m(B)}{\int_Bw(x)dx},\quad 1<t<1+\frac1{|R|\cdot\max_{v\in R}k(v)}.$$
