...
\section{Intermediate upper-bound results}

\begin{theorem} Let $\Gamma$ be a chamber. Let $V$ be an integral operator on $L_2(\Gamma,w(x)dx)$ whose integral kernel $K$ satisfies the condition
...
\item
$$\|e_{k,l,I}\|_{L_r(\mathbb{R}^N,w(x)dx)}=\|f\|_{L_r(\mathbb{R}^N,w(x)dx)},\quad \|f_{k,l,I}\|_{L_r(\mathbb{R}^N,w(x)dx)}\leq c_{K,\kappa}d_Iw(I)^{\frac1r-1};$$
...
$$=\sum_{\substack{I\in\mathbb{D}_{\Gamma}\\ 3I\subset\Gamma}}\chi_I(x)\chi_{3I}(y)K(x,y)\psi(\frac{\Phi(x)-\Phi(y)}{d_I})+$$
$$+\sum_{\substack{I\in\mathbb{D}_{\Gamma}\\ 3I\not\subset\Gamma}}\chi_I(x)\chi_{3I}(y)K(x,y)\psi(\frac{\Phi(x)-\Phi(y)}{d_I}).$$

We will only consider the first sum on the right hand side. The second some can be treated similarly, but the notations become heavier. {\color{red} feel free to write the expression for the second sum if you think it is needed. the only difference is that the domain of the map $K_I$ becomes more complicated.}
...
$$\|K_I\|_{C^{4N+2}([-\frac12,\frac12]^N\times[-\frac32,\frac32]^N)}\leq c_K\frac{d_I}{(d_I+|c_I-gc_I|)\cdot w(I)}.$$
Hence, there exists an extension $K_I:\mathbb{T}^N\times\mathbb{T}^N\to\mathbb{C}$ such that
$$\|K_I\|_{C^{4N+2}(\mathbb{T}^N\times\mathbb{T}^N)}\leq c_K'\frac{d_I}{(d_I+|c_I-gc_I|)\cdot w(I)}.$$
In particular,
$$\|K_I\|_{W^{4N+2,2}(\mathbb{T}^N\times\mathbb{T}^N)}\leq c_K''\frac{d_I}{(d_I+|c_I-gc_I|)\cdot w(I)}.$$
We may, therefore, write
$$K_I(a,b)=\sum_{k,l\in\mathbb{Z}^N}{\rm coef}_{k,l,I}e^{i\langle k,a\rangle}e^{i\langle l,b\rangle},\quad a,b\in\mathbb{T}^N,$$
where
$$\sum_{k,l\in\mathbb{Z}^N}(1+|k|^2+|l|^2)^{4N+2}|{\rm coef}_{k,l,I}|^2\leq (c_K''\frac{d_I}{(d_I+|c_I-gc_I|)\cdot w(I)})^2.$$
In particular,
$$|{\rm coef}_{k,l,I}|\leq c_K''(1+|k|^2+|l|^2)^{-2N-1}\frac{d_I}{(d_I+|c_I-gc_I|)\cdot w(I)}.$$

This allows us to write
$$\sum_{\substack{I\in\mathbb{D}_{\Gamma}\\ 3I\subset\Gamma}}\chi_I(x)\chi_{3I}(y)K(x,y)\psi(\frac{\Phi(x)-\Phi(y)}{d_I})=$$
$$=\sum_{\substack{I\in\mathbb{D}_{\Gamma}\\ 3I\subset\Gamma}}\chi_I(x)\chi_{3I}(y)K_I(\frac{\Phi^{-1}(x-c_I)}{d_I},\frac{\Phi^{-1}(x)-c_I}{d_I})=$$
...
In those notations,
$$f(x)\cdot\sum_{\substack{I\in\mathbb{D}_{\Gamma}\\ 3I\subset\Gamma}}\chi_I(x)\chi_{3I}(y)K(x,y)\psi(\frac{\Phi(x)-\Phi(y)}{d_I})=$$
$$=\sum_{k,l\in\mathbb{Z}^N}(1+|k|^2+|l|^2)^{-2N-1}\sum_{\substack{I\in\mathbb{D}_{\Gamma}\\ 3I\subset\Gamma}}e_{k,l,I}(x)f_{k,l,I}(y).$$

Obviously,
$${\rm supp}(e_{k,l,I})\subset I,\quad {\rm supp}(f_{k,l,I})\subset 3I,$$
$$\|e_{k,l,I}\|_{L_r(\mathbb{R}^N,w(x)dx)}=\|f\|_{L_r(I,w(x)dx)},$$
$$\|f_{k,l,I}\|_{L_r(\mathbb{R}^N,w(x)dx)}=(1+|k|^2+|l|^2)^{2N+1}|{\rm coef}_{I,k,l}|\cdot w(3I)^{\frac1r}\leq$$
$$\leq c_K''\frac{d_I}{(d_I+|c_I-gc_I|)\cdot w(I)}\cdot w(3I)^{\frac1r}\leq c_K'''\frac{d_I}{d_I+|c_I-gc_I|}w(I)^{\frac1r-1}.$$
\end{proof}

\begin{lemma}\label{intermediate cwikel estimate} We have
$$\|M_fV\|_{\mathcal{L}_{N,\infty}(L_2(\mathbb{R}^N,w(x)dx))}\leq c_{K,\kappa}\Big\|\Big\{\frac{d_I}{d_I+|c_I-gc_I|}w(I)^{-\frac1r}\|f\|_{L_r(I,w(x)dx)}\Big\}_{I\in\mathbb{D}_{\Gamma}}\Big\|_{l_{N,\infty}}.$$
\end{lemma}
...
\begin{lemma} Let $1_G\neq g\in G.$ We have
$$\Big\|\Big\{\frac{d_I}{d_I+|c_I-gc_I|}w(I)^{-\frac1r}\|f-f\circ g\|_{L_r(I,w(x)dx)}\Big\}_{I\in\mathbb{D}_{\Gamma}}\Big\|_{l_{N,\infty}}\leq$$
$$\leq c_{\kappa,\Phi,r}\Big(\Big\|\Big\{|I|^{-\frac1r}\|f-f_I\|_{L_r(I)}\Big\}_{I\in\mathbb{D}_{\Gamma}}\Big\|_{l_{N,\infty}}+$$
$$+\Big\|\Big\{|I|^{-\frac1r}\|f\circ g-(f\circ g)_I\|_{L_r(I)}\Big\}_{I\in\mathbb{D}_{\Gamma}}\Big\|_{l_{N,\infty}}+\Big\|\Big\{d_I|I|^{-1}\|H_g\|_{L_1(I)}\Big\}_{I\in\mathbb{D}_{\Gamma}}\Big\|_{l_{N,\infty}}\Big).$$
\end{lemma}
\begin{proof} Denote by $f_{I,w}$ and, respectively, $(f\circ g)_{I,w}$ the averaging (with respect to measure $w(x)dx$) of $f$ (respectively, of $f\circ g$) over $I.$ We, obviously, have
$$\|f-f\circ g\|_{L_r(I,w(x)dx)}\leq\|f-f_{I,w}\|_{L_r(I,w(x)dx)}+$$
$$+\|f\circ g-(f\circ g)_{I,w}\|_{L_r(I,w(x)dx)}+|f_{I,w}-(f\circ g)_{I,w}|\cdot w(I)^{\frac1r}.$$
Obviously,
$$|f_{I,w}-(f\circ g)_{I,w}|\leq\|f-f\circ g\|_{L_1(I,w(x)dx)}\cdot w(I)^{-1}$$
and
$$\|f-f_{I,w}\|_{L_r(I,w(x)dx)}\leq 2\inf_{c\in\mathbb{R}}\|f-c\|_{L_r(I,w(x)dx)}\leq2\|f-f_I\|_{L_r(I,w(x)dx)},$$
$$\|f\circ g-(f\circ g)_{I,w}\|_{L_r(I,w(x)dx)}\leq 2\inf_{c\in\mathbb{R}}\|f\circ g-c\|_{L_r(I,w(x)dx)}\leq2\|f\circ g-(f\circ g)_I\|_{L_r(I,w(x)dx)}.$$
Therefore,
$$\frac{d_I}{d_I+|c_I-gc_I|}w(I)^{-\frac1r}\|f-f\circ g\|_{L_r(I,w(x)dx)}\leq$$
$$\leq 2w(I)^{-\frac1r}\|f-f_I\|_{L_r(I,w(x)dx)}+2w(I)^{-\frac1r}\|f\circ g-(f\circ g)_I\|_{L_r(I,w(x)dx)}+$$
$$+\frac{d_I}{d_I+|c_I-gc_I|}w(I)^{-1}\|f-f\circ g\|_{L_1(I,w(x)dx)}.$$

Using Lemma \ref{supremum vs average}, we write
$$w(I)^{-\frac1r}\|f-f_I\|_{L_r(I,w(x)dx)}\leq c_{\kappa,\Phi}^{\frac1r}|I|^{-\frac1r}\|f-f_I\|_{L_r(I)},$$
$$w(I)^{-\frac1r}\|f\circ g-(f\circ g)_I\|_{L_r(I,w(x)dx)}\leq c_{\kappa,\Phi}^{\frac1r}|I|^{-\frac1r}\|f\circ g-(f\circ g)_I\|_{L_r(I)}.$$

Obviously,
$$|x-gx|\leq|x-c_I|+|gx-gc_I|+|c_I-gc_I|\leq c_{\kappa,\Phi}'(d_I+|c_I-gc_I|),\quad x\in I.$$
Set
$$H_g(x)=\frac{|f(x)-f(gx)|}{|x-gx|},\quad x\in I.$$
We have
$$\frac1{d_I+|c_I-gc_I|}|f-f\circ g|\leq c_{\kappa,\Phi}''H_g.$$
Thus,
$$\frac1{d_I+|c_I-gc_I|}\|f-f\circ g\|_{L_1(I,w(x)dx)}\leq c_{\kappa,\Phi}''\|H_g\|_{L_1(I,w(x)dx)}.$$
Using Lemma \ref{supremum vs average}, we conclude
$$\frac1{d_I+|c_I-gc_I|}w(I)^{-1}\|f-f\circ g\|_{L_1(I,w(x)dx)}\leq c_{\kappa,\Phi}'''\frac1{|I|}\|H_g\|_{L_1(I)}.$$
