...
$$|\partial_x^{\alpha}\partial_y^{\beta}K(x,y)|\leq C_{\alpha,\beta}\frac{|x-y|^{1-|\alpha|_1-|\beta|_1}}{V(x,y,|x-y|)}.$$
We have
...
\]
be the dyadic system over $\Gamma$ constructed in
Corollary~\ref{Construction over gamma}.
If
...
Let $K_{(-\Delta_{\D})^{-1/2}}(x,y)$ denote the integral kernel of
$(-\Delta_{\D})^{-1/2}$.

For $\sigma\in G,I\in\mathbb D_\Gamma$, and for every
$f\in L^r_{\mathrm{loc}}(\mathbb R^N,dw)$, define
...
\begin{lemma}\label{lem:rank-one-decomposition-Dunkl} Assume that $N>1$ and $2<r<\infty.$ There exist a sequence
$\{c_{p,q}\}_{p,q\in\mathbb{Z}^N}\in l_1(\mathbb{Z}^N\times\mathbb{Z}^N),$ functions
$
\{e_{p,q,I}\}_{p,q\in\mathbb{Z}^N,I\in\mathbb{D}_{\Gamma}},$ $
\{h_{p,q,I}\}_{p,q\in\mathbb{Z}^N,I\in\mathbb{D}_{\Gamma}},$
such that
\begin{enumerate}
\item  \begin{equation}\label{eq:Dunkl-support-e-new}
\operatorname{supp}(e_{p,q,I})\subset I,\qquad
\operatorname{supp}(h_{p,q,I})
\subset 2I\cap\Gamma;
\end{equation}
\item \begin{equation}\label{eq:Dunkl-normalization-e-new}
\|e_{p,q,I}\|_{L_r(\mathbb{R}^N,w(x)dx)}\leq
w(I)^{\frac1r-\frac12},\quad \|h_{p,q,I}\|_{L_r(\mathbb{R}^N,w(x)dx)}\leq w(I)^{\frac1r-\frac12};
\end{equation}
\item
For every $f\in L_r^{\mathrm{loc}}(\mathbb{R} ^N,w(x)dx)$ and for almost every $x,y\in\Gamma$ we have
\begin{equation}\label{eq:Dunkl-kernel-rank-one-new}
f(x)K(x,y)=\sum_{p,q\in\mathbb{Z}^N}c_{p,q}
\sum_{I\in\mathbb{D}_{\Gamma}}\lambda_I(f)e_{p,q,I}(x)h_{p,q,I}(y).
\end{equation}
\end{enumerate}
...
Set
$
x_\sigma:=\sigma^{-1}x\in\Gamma^\circ,
$ $
...
v:=\Phi(y_\tau).
\]
Since $x_\sigma$ and $y_\tau$ belong to the same closed Weyl chamber,
...
\begin{equation}\label{eq:rankone-d-Phi}
C_\Phi^{-1}|u-v|
\leq
d(x,y)
\leq
...
\qquad
\rho(z)=1
\quad\text{for }|z|\leq\frac14,
\]
and
\[
\rho(z)=0
\quad\text{for }|z|\geq\frac12.
\]
Set
$
\psi(z):=\rho(z)-\rho(2z).
...
\end{equation}
Indeed, for $m,n\in\mathbb N$,
\[
\sum_{j=-m}^{n}\psi(2^jz)
=
\rho(2^{-m}z)-\rho(2^{n+1}z),
\]
and \eqref{eq:rankone-psi-partition} follows by letting
$m,n\to\infty$.

Let $j\in\mathbb Z$. For almost every $u\in\Omega$, there is a unique
cube
$
Q\in\mathbb D^0_{\Omega,j}
$
containing $u$. If
$
I:=\Phi^{-1}(Q)\in\mathbb D_{\Gamma,j},
$
then $\ell(I)=2^{-j}$. Hence, whenever $d(x,y)>0$, we have
$u\neq v$ by \eqref{eq:rankone-d-Phi}, and therefore
\begin{align}
K_{(-\Delta_{\D})^{-1/2}}(x,y)
&=
\sum_{j\in\mathbb Z}
...
\right).
\label{eq:rankone-shell-decomposition}
\end{align}
For every fixed pair $(x,y)$ with $d(x,y)>0$, only finitely many
values of $j$ occur in \eqref{eq:rankone-shell-decomposition}.

We next record the localization of the $y$-variable. Let
\[
I=\Phi^{-1}(Q),
\qquad
\ell:=\ell(I),
\]
and suppose that $u\in Q$ and
\[
\psi\left(\frac{u-v}{\ell}\right)\neq0.
\]
By \eqref{eq:rankone-psi-support},
\[
|u-v|\leq\frac{\ell}{2}.
\]
Let $c_Q$ denote the center of $Q$. Since $u\in Q$, for every
$1\leq j\leq N$,
\[
|u_j-(c_Q)_j|\leq\frac{\ell}{2}.
\]
Moreover,
\[
|u_j-v_j|
...
\begin{equation}\label{eq:rankone-y-localization}
y\in\tau I^\ast.
\end{equation}

We show that
\begin{equation}\label{eq:rankone-Istar-I}
w(I^\ast)\simeq w(I).
\end{equation}
Put
\[
z_I:=\Phi^{-1}(c_Q)\in\Gamma.
\]
By \eqref{eq:biLip-N}, there exist constants $c_1,C_1>0$,
independent of $I$, such that
\[
B_\Gamma(z_I,c_1\ell)
\subset I
\subset I^\ast
\subset B_\Gamma(z_I,C_1\ell).
\]
Indeed, the first inclusion follows by taking $c_1>0$ sufficiently
small, whereas the last one follows from
\[
(2Q)\cap\Omega
\subset B(c_Q,\sqrt N\,\ell)
\]
and the boundedness of $\Phi^{-1}$. By the doubling property,
\[
w(I^\ast)
\leq
w(B_\Gamma(z_I,C_1\ell))
\lesssim
w(B_\Gamma(z_I,c_1\ell))
\leq
w(I).
\]
Since $I\subset I^\ast$, the reverse inequality is immediate, and
\eqref{eq:rankone-Istar-I} follows.
...
$I=\Phi^{-1}(Q)\in\mathbb D_\Gamma$, and write
$
\ell:=\ell(I).
$
...
a\in [-1/2,1/2]^N.
$
If $v\in(2Q)\cap\Omega$, then $b$ belongs to
\[
B_Q
:=
\left\{
...
\[
m_i\in\mathbb Z_+
\quad (1\leq i\leq N_0),
\qquad
m_i\in\mathbb Z
\quad (N_0<i\leq N).
\]
For $1\leq i\leq N_0$, the $i$-th component of $B_Q$ is
\[
\begin{cases}
[-1/2,1],&m_i=0,\\
[-1,1],&m_i\geq1,
\end{cases}
\]
whereas for $i>N_0$ it is always $[-1,1]$. Hence the rectangles
$
[-1/2,1/2]^N\times B_Q
$
belong to a finite family.

Consequently, for every $Q$ we may choose
$
\chi_Q\in
C_c^\infty((-2,2)^N\times(-2,2)^N)
$
such that
\[
\chi_Q=1
...
[-1/2,1/2]^N\times B_Q,
\]
and, for some sufficiently small fixed $\varepsilon_0>0$,
\[
...
\right\}.
\]
Since only finitely many such rectangles occur, these functions may
be chosen so that, for every integer $M\geq0$,
\begin{equation}\label{eq:rankone-chi-uniform}
\sup_Q
\|\chi_Q\|_{C^M(\mathbb R^{2N})}
<\infty.
\end{equation}

We choose $\varepsilon_0$ sufficiently small so that
\begin{equation}\label{eq:rankone-distance-scale}
d\left(
...
\[
|a-a_0|+|b-b_0|
\lesssim\varepsilon_0.
\]
Since $a-b\in\operatorname{supp}\psi$,
\[
|a-b|\geq\frac18.
\]
Thus, after decreasing $\varepsilon_0$ if necessary,
\[
|a_0-b_0|\geq\frac1{16}.
\]
Set
\[
x_0'
...
Since
\[
c_Q+\ell a_0,\quad c_Q+\ell b_0\in\Omega,
\]
we have $x_0',y_0'\in\Gamma$. Hence, by
\eqref{distanceequa} and \eqref{eq:biLip-N},
\[
d(\sigma x_0',\tau y_0')
=
d(x_0',y_0')
...
\gtrsim\ell.
\]
Moreover, directly from the definition of $d$,
\begin{equation}\label{eq:rankone-d-Lipschitz}
|d(x,y)-d(x',y')|
\leq
\|x-x'\|+\|y-y'\|.
\end{equation}
Since $\Phi^{-1}$ is bounded,
\[
\left\|
\Phi^{-1}(c_Q+\ell a)
-
...
\varepsilon_0\ell,
\]
and similarly for $b$ and $b_0$. Therefore,
\eqref{eq:rankone-d-Lipschitz} gives the lower bound in
\eqref{eq:rankone-distance-scale} if $\varepsilon_0$ is sufficiently
small.

For the reverse estimate, we use the $G$-invariance of the orbit
distance in both variables:
\[
d(\sigma x,\tau y)=d(x,y),
...
$|a-b|\leq 1/2$ on $\operatorname{supp}\psi$.
Set
\[
X:=
\sigma\Phi^{-1}(c_Q+\ell a),
...
\tau\Phi^{-1}(c_Q+\ell b).
\]
For $(a,b)$ in the above support, $X$ and $Y$ lie within distance
$C\ell$ of $\sigma z_I$ and $\tau z_I$, respectively. Combining
\eqref{eq:rankone-distance-scale}, the doubling estimate,
the $G$-invariance of $dw$, and \eqref{globallocalvol}, we obtain
\[
w(B(X,d(X,Y)))
\simeq
w(B(z_I,\ell))
...
\]
Consequently,
\begin{equation}\label{eq:rankone-V-I}
V(X,Y,d(X,Y))
\simeq
w(I).
\end{equation}

By Lemma~\ref{lem:Dunkl-fractional-kernel-derivative},
...
\]
Combining this with
\eqref{eq:rankone-distance-scale} and
\eqref{eq:rankone-V-I}, we obtain
\begin{equation}\label{eq:rankone-localized-derivative}
\left|
\partial_x^\alpha\partial_y^\beta
K_{(-\Delta_{\D})^{-1/2}}(X,Y)
\right|
...
\begin{equation}\label{eq:rankone-H-CM}
\sup_{\sigma,\tau\in G}
\sup_{I\in\mathbb D_\Gamma}
\|H_{\sigma,\tau,I}\|_{C^M(\mathbb R^{2N})}
<\infty.
\end{equation}
Indeed, if altogether $k$ derivatives fall on the kernel term, the
chain rule produces a factor bounded by a constant multiple of
$\ell(I)^k$. By \eqref{eq:rankone-localized-derivative},
...
\lesssim1.
\]
The derivatives falling on $\chi_Q$ and $\psi$ are uniformly bounded
by \eqref{eq:rankone-chi-uniform} and the fact that $\psi$ is fixed.
The product rule proves \eqref{eq:rankone-H-CM}.

Since $H_{\sigma,\tau,I}$ is compactly supported in the interior of
$
[-2,2]^N\times[-2,2]^N,
$
its $4$-periodic extension is smooth. Hence
\begin{equation}\label{eq:rankone-Fourier}
H_{\sigma,\tau,I}(a,b)
=
\sum_{p,q\in\mathbb Z^N}
b_{p,q,I}^{\sigma,\tau}
...
\eqref{eq:rankone-H-CM} yield, for every $M>0$,
\begin{equation}\label{eq:rankone-b-rapid}
\sup_{\sigma,\tau\in G}
\sup_{I\in\mathbb D_\Gamma}
|b_{p,q,I}^{\sigma,\tau}|
\leq
...
When $x\in\sigma I$ and $y\in\tau I^\ast$, the corresponding
normalized variables satisfy
\[
a\in[-1/2,1/2]^N,
\qquad
b\in B_Q,
\]
and therefore $\chi_Q(a,b)=1$. Thus
\begin{align}
&K_{(-\Delta_{\D})^{-1/2}}(x,y)
\psi\left(
\frac{
...
\bigl(\Phi(\tau^{-1}y)-c_Q\bigr)
\right).
\label{eq:rankone-local-K-Fourier}
\end{align}

Define
\begin{equation}\label{eq:rankone-cpq}
c_{p,q}
:=
...
\right\}.
\end{equation}
By \eqref{eq:rankone-Istar-I} and
\eqref{eq:rankone-b-rapid}, for every $M>0$,
\[
c_{p,q}
\leq
C_M(1+|p|+|q|)^{-M}.
\]
Thus \eqref{eq:Dunkl-cpq-decay} holds.
...
\]
Then
\begin{equation}\label{eq:rankone-a-bound}
|a_{p,q,I}^{\sigma,\tau}|\leq1,
\end{equation}
and
\begin{equation}\label{eq:rankone-b-factor}
b_{p,q,I}^{\sigma,\tau}
=
c_{p,q}a_{p,q,I}^{\sigma,\tau}
\left(
\frac{w(I)}{w(I^\ast)}
\right)^{1/2}.
\end{equation}
...
w(\tau I^\ast)=w(I^\ast).
\]
By \eqref{eq:rankone-a-bound}, if
$\|f\|_{L^r(\sigma I,dw)}>0$, then
\begin{align*}
\|e_{p,q,I}^{\sigma,\tau}\|_{L^r(dw)}
\leq
w(I)^{\frac1r-\frac12}
...
w(I)^{\frac1r-\frac12}.
\end{align*}
The same estimate is trivial when
$\|f\|_{L^r(\sigma I,dw)}=0$. Hence
\[
...
\]
Since $I\subset I^\ast$, this yields
\[
w(I^\ast)^{\frac1r-\frac12}
\leq
w(I)^{\frac1r-\frac12}.
\]
Therefore,
\begin{equation}\label{eq:rankone-normalization-proof}
\|e_{p,q,I}^{\sigma,\tau}\|_{L^r(\mathbb R^N,dw)}
\leq
w(I)^{\frac1r-\frac12},
\qquad
...
w(I)^{\frac1r-\frac12},
\end{equation}
which is precisely \eqref{eq:Dunkl-normalization-e-new}.
...
\|f\|_{L^r(\sigma I,dw)}>0.
$
By the definition of $\lambda_{\sigma,I}(f)$,
\eqref{eq:rankone-e}, \eqref{eq:rankone-h}, and
\eqref{eq:rankone-b-factor}, for $x\in\sigma I$ and
$y\in\tau I^\ast$ we have
\begin{align*}
&c_{p,q}\lambda_{\sigma,I}(f)
e_{p,q,I}^{\sigma,\tau}(x)
h_{p,q,I}^{\sigma,\tau}(y)
\\
&=
...
\right).
\end{align*}
By \eqref{eq:rankone-b-factor},
\[
c_{p,q}a_{p,q,I}^{\sigma,\tau}
w(I)^{-1/2}w(I^\ast)^{-1/2}
=
b_{p,q,I}^{\sigma,\tau}w(I)^{-1}.
\]
Therefore,
\begin{align*}
&c_{p,q}\lambda_{\sigma,I}(f)
...
\right).
\end{align*}
Summing over $p,q$ and using
\eqref{eq:rankone-local-K-Fourier}, we obtain
\begin{align}
...
\|f\|_{L^r(\sigma I,dw)}=0,
$
then $f=0$ almost everywhere on $\sigma I$, while
$e_{p,q,I}^{\sigma,\tau}=0$. Hence
\eqref{eq:rankone-one-piece} remains valid for almost every
$x\in\sigma I$.

If $x\in\sigma I$ but $y\notin\tau I^\ast$, then
\eqref{eq:rankone-y-localization} implies that the left-hand side of
\eqref{eq:rankone-one-piece} vanishes, while
$
h_{p,q,I}^{\sigma,\tau}(y)=0.
$
Thus \eqref{eq:rankone-one-piece} holds for almost every
$
(x,y)\in\sigma I\times\tau(\Gamma).
$

We next justify the absolute convergence needed to rearrange the
series. Fix $\sigma,\tau$ and almost every $(x,y)$ such that
\[
x\in\sigma(\Gamma)^\circ,
\qquad
y\in\tau(\Gamma)^\circ,
...
d(x,y)>0.
\]
If a term indexed by $I$ is nonzero, then
\[
x\in\sigma I,
\qquad
y\in\tau I^\ast.
\]
The geometry of $I$ and $I^\ast$, together with
\eqref{distanceequa} and \eqref{eq:biLip-N}, gives
\[
d(x,y)\lesssim\ell(I).
\]
For each fixed generation there is at most one
$I\in\mathbb D_\Gamma$ containing $\sigma^{-1}x$.

Moreover, from
\[
B_\Gamma(z_I,c_1\ell(I))\subset I
\]
and the volume estimates in the text,
\[
w(I)
\gtrsim
\ell(I)^\N.
...
\eqref{eq:rankone-b-rapid}, we obtain, for every $M>0$,
\begin{align*}
&c_{p,q}\lambda_{\sigma,I}(f)
\left|
e_{p,q,I}^{\sigma,\tau}(x)
...
\ell(I)^{1-\N}.
\end{align*}
Since $\N>1$, the sum over all dyadic scales satisfying
\[
\ell(I)\gtrsim d(x,y)
\]
converges. Choosing $M>2N$ also gives
\[
\sum_{p,q\in\mathbb Z^N}
(1+|p|+|q|)^{-M}<\infty.
\]
Hence the relevant series is absolutely convergent for almost every
such $(x,y)$.

Summing \eqref{eq:rankone-one-piece} first over
$I\in\mathbb D_{\Gamma,j}$ and then over $j\in\mathbb Z$, and using
\eqref{eq:rankone-shell-decomposition}, gives
\[
f(x)K_{(-\Delta_{\D})^{-1/2}}(x,y)
=
\sum_{p,q\in\mathbb Z^N}
...
Finally, by the Weyl chamber decomposition established above in the
text, the interiors of the chambers $\sigma(\Gamma)$ cover
$\mathbb R^N$ up to the reflecting hyperplanes, which form a
$dw$-null set. Summing over $\sigma,\tau\in G$, we obtain
\[
f(x)K_{(-\Delta_{\D})^{-1/2}}(x,y)
=
\sum_{\sigma,\tau\in G}
...
Thus \eqref{eq:Dunkl-kernel-rank-one-new} holds, and the proof is
complete.
\end{proof}
