...
\begin{definition} Let $\mathscr{D}$ be a system of dyadic cubes. A sequence of functions $\{e_Q\}_{Q\in\mathscr{D}}$ is an NWO sequence if
\begin{enumerate}
\item $e_Q$ is supported in $cQ$ for some positive constant $c\in 2\mathbb{N}+1$ independent of $Q;$
\item $\|e_Q\|_{L_r(\mathbb{R}^{N},w(x)dx)}\leq w(Q)^{\frac1r-\frac12}$ for some $r>2;$
\end{enumerate}
\end{definition}
...
Corollary \ref{Construction over gamma}. If $I\in\mathbb{D}_{\Gamma}$ and if $Q=\Phi(I),$ then we denote
$$d_I=\ell(Q).$$
\end{nota}
...
$$\|f_{k,l,I}\|_{L_r(\mathbb{R}^N,w(x)dx)}=(1+|k|^2+|l|^2)^{2N+1}|{\rm coef}_{I,k,l}|\cdot w(3I)^{\frac1r}\leq c_K''\frac{d_I}{w(I)}\cdot w(3I)^{\frac1r}\leq c_{K,\kappa}d_Iw(I)^{\frac1r-1}.$$
\end{proof}

\begin{lemma}\label{intermediate cwikel estimate} We have
$$\|M_fV\|_{\mathcal{L}_{N,\infty}(L_2(\mathbb{R}^N,w(x)dx))}\leq c_{K,\kappa}\Big\|\Big\{d_Iw(I)^{-\frac1r}\|f\|_{L_r(I,w(x)dx)}\Big\}_{I\in\mathbb{D}_{\Gamma}}\Big\|_{l_{N,\infty}}.$$
\end{lemma}
\begin{proof} The assertion follows from Lemma \ref{lem:rank-one-decomposition-Dunkl}, Lemma \ref{NWOLEMMAA2} and the triangle inequality. {\color{red} feel free to expand the argument if needed.}
\end{proof}
...
\begin{lemma}\label{weighted vs flat} For every $I\in\mathbb{D}_{\Gamma},$ we have
$$w(I)^{-\frac1r}\|f\|_{L_r(I,w(x)dx)}\leq c_{\kappa,\Phi}|I|^{-\frac1r}\|f\|_{L_r(I)}.$$
\end{lemma}
\begin{proof} The assertion follows immediately from Lemma \ref{supremum vs average}.
\end{proof}

\begin{lemma}\label{second intermediate cwikel estimate} We have
$$\|M_fV\|_{\mathcal{L}_{N,\infty}(L_2(\mathbb{R}^N,w(x)dx))}\leq c_{K,\kappa,\Phi}\Big\|\Big\{d_I^{1-\frac{N}{r}}\|f\|_{L_r(I)}\Big\}_{I\in\mathbb{D}_{\Gamma}}\Big\|_{l_{N,\infty}}.$$
\end{lemma}
\begin{proof} The assertion follows by combining Lemma \ref{intermediate cwikel estimate} and Lemma \ref{weighted vs flat}.
\end{proof}
