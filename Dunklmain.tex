...
\section{Alternative proof by using Dima's approach}
\begin{lemma}\label{lem:Weyl-off-diagonal-kernel-NWO}
Fix a closed Weyl chamber \(\Gamma\) and
\(\sigma\in G\setminus\{\mathrm{Id}\}\). Suppose that \(K\) satisfies
\begin{align*}
\bigl|
\partial_x^\alpha\partial_y^\beta K(x,y)
\bigr|
\lesssim_{\alpha,\beta}
\frac{\|x-y\|^{1-|\alpha|-|\beta|}}
{\|x-\sigma y\|V\bigl(x,y,\|x-y\|\bigr)}
\end{align*}
for every pair of multi-indices \(\alpha,\beta\) and every
\(x,y\in\Gamma^\circ\) with \(x\neq y\). For every \(M_0>2N\), there
exist positive numbers \(\{c_{u,v}\}_{u,v\in\mathbb Z^N}\) and
families
\(\{e_{u,v,I}\}_{u,v\in\mathbb Z^N,I\in\mathbb D_\Gamma}\) and
\(\{h_{u,v,I}\}_{u,v\in\mathbb Z^N,I\in\mathbb D_\Gamma}\) such that
\begin{align}
|c_{u,v}|
&\lesssim_{M_0}
(1+|u|+|v|)^{-M_0},
\label{eq:Weyl-off-diagonal-Fourier-decay}\\
\operatorname{supp}e_{u,v,I}\subset I,
\qquad
\operatorname{supp}h_{u,v,I}
&\subset I^*,
\label{eq:Weyl-off-diagonal-support}\\
|e_{u,v,I}|+|h_{u,v,I}|
&\lesssim w(I)^{-1/2},
\label{eq:Weyl-off-diagonal-size}
\end{align}
and, for almost every \(x,y\in\Gamma\) with \(x\neq y\),
\begin{align}
K(x,y)
=
\sum_{u,v\in\mathbb Z^N}
...
e_{u,v,I}(x)h_{u,v,I}(y).
\label{eq:Weyl-off-diagonal-NWO-expansion}
\end{align}
\end{lemma}
...
\subset
\biggl\{
z:\frac18\leq |z|\leq\frac12
\biggr\},
\qquad
\sum_{m\in\mathbb Z}\psi(2^mz)=1,
\quad z\neq0.
\label{eq:Weyl-off-diagonal-partition}
\end{align}
For \(x,y\in\Gamma^\circ\), \(x\neq y\), the unique cube
\(I\in\mathbb D_{\Gamma,m}\) containing \(x\) at each scale satisfies
\begin{align}
K(x,y)
=
\sum_{I\in\mathbb D_\Gamma}
\chi_I(x)K(x,y)
\psi\biggl(
\frac{\Phi(x)-\Phi(y)}{\ell(I)}
\biggr).
\label{eq:Weyl-off-diagonal-localization}
\end{align}
If the \(I\)-th summand is nonzero, then
\(|\Phi(x)-\Phi(y)|\leq\ell(I)/2\); hence \(y\in I^*\).

Fix \(I=\Phi^{-1}(Q)\). For
\[
a\in\bigl(-1/2,1/2\bigr)^N,
\qquad
c_Q+\ell(I)b\in(2Q)\cap\Omega,
\]
set
\[
X:=\Phi^{-1}\bigl(c_Q+\ell(I)a\bigr),
\qquad
Y:=\Phi^{-1}\bigl(c_Q+\ell(I)b\bigr).
\]
Whenever \(\psi(a-b)\neq0\), by the \(G\)-invariance of \(d\) and
\(dw\), \eqref{distanceequa}, and Lemma
\ref{lem:Dunkl-Riesz-endpoint-local-geometry} applied with
\((\mathrm{Id},\sigma)\),
\begin{align}
\|X-Y\|
&\simeq\ell(I),\nonumber\\
\|X-\sigma Y\|
&\simeq\ell(I)+|z_I-\sigma z_I|,\nonumber\\
V\bigl(X,Y,\|X-Y\|\bigr)
&\simeq w(I).
\label{eq:Weyl-off-diagonal-local-geometry}
\end{align}
Consequently,
\begin{align}
\bigl|
...
\lesssim_{\alpha,\beta}
\theta_{\mathrm{Id},\sigma,I}
\frac{\ell(I)^{-|\alpha|-|\beta|}}{w(I)}.
\label{eq:Weyl-off-diagonal-local-derivative}
\end{align}
...
\biggr\}.
\]
On \(\mathcal R_Q\), define
\begin{align*}
F_I(a,b)
:=
\frac{w(I)}{\theta_{\mathrm{Id},\sigma,I}}
\psi(a-b)
K\biggl(
\Phi^{-1}\bigl(c_Q+\ell(I)a\bigr),
\Phi^{-1}\bigl(c_Q+\ell(I)b\bigr)
\biggr).
\end{align*}
By \eqref{eq:Weyl-off-diagonal-local-derivative} and the chain rule,
for every integer \(L\geq0\),
\begin{align}
\sup_{I\in\mathbb D_\Gamma}
\|F_I\|_{W^{L,\infty}(\mathcal R_Q)}
\lesssim_L1.
\label{eq:Weyl-off-diagonal-normalized-derivative}
\end{align}
The rectangles \(\mathcal R_Q\) range over finitely many fixed
rectangular domains. Fix an integer \(L>M_0\). By the standard
extension theorem for Sobolev spaces on rectangles, followed by
multiplication by a fixed smooth cutoff, \(F_I\) admits an extension
\(\widetilde F_I\) supported in
\((-2,2)^N\times(-2,2)^N\) such that
\begin{align}
\widetilde F_I=F_I
\quad\text{on }\mathcal R_Q,
\qquad
\sup_{I\in\mathbb D_\Gamma}
\|\widetilde F_I\|_{W^{L,\infty}(\mathbb R^{2N})}
\lesssim_L1.
\label{eq:Weyl-off-diagonal-extension}
\end{align}

The \(4\)-periodic extension of \(\widetilde F_I\) has a Fourier
expansion
\begin{align*}
\widetilde F_I(a,b)
=
...
e^{\frac{\pi i}{2}v\cdot b}.
\end{align*}
By repeated integration by parts and
\eqref{eq:Weyl-off-diagonal-extension},
\begin{align*}
\sup_{I\in\mathbb D_\Gamma}
|b_{u,v,I}|
\lesssim_L
(1+|u|+|v|)^{-L}.
\end{align*}
Increasing the implicit constant if necessary, define
\begin{align*}
c_{u,v}
:=
C_{M_0}(1+|u|+|v|)^{-M_0}
\end{align*}
so that
\(|b_{u,v,I}|\leq c_{u,v}\) for every \(I\). For
\(I=\Phi^{-1}(Q)\), define
\begin{align*}
e_{u,v,I}(x)
&:=
\frac{b_{u,v,I}}{c_{u,v}}
w(I)^{-1/2}\chi_I(x)
\exp\biggl(
\frac{\pi i}{2\ell(I)}
u\cdot\bigl(\Phi(x)-c_Q\bigr)
\biggr),\\
h_{u,v,I}(y)
&:=
w(I)^{-1/2}\chi_{I^*}(y)
\exp\biggl(
\frac{\pi i}{2\ell(I)}
v\cdot\bigl(\Phi(y)-c_Q\bigr)
\biggr).
\end{align*}
Then \eqref{eq:Weyl-off-diagonal-Fourier-decay}--\eqref{eq:Weyl-off-diagonal-size}
hold.

If \(x\in I\) and \(y\in I^*\), then
\[
a:=\frac{\Phi(x)-c_Q}{\ell(I)},
\qquad
b:=\frac{\Phi(y)-c_Q}{\ell(I)}
\]
belong to \(\mathcal R_Q\). Hence, by
\eqref{eq:Weyl-off-diagonal-extension} and the Fourier expansion,
\begin{align}
K(x,y)
\psi\biggl(
\frac{\Phi(x)-\Phi(y)}{\ell(I)}
...
=
\sum_{u,v\in\mathbb Z^N}
c_{u,v}\theta_{\mathrm{Id},\sigma,I}
e_{u,v,I}(x)h_{u,v,I}(y).
\label{eq:Weyl-off-diagonal-local-Fourier}
\end{align}
If \(x\in I\) and \(y\notin I^*\), both sides of
\eqref{eq:Weyl-off-diagonal-local-Fourier} vanish. Therefore,
\begin{align*}
\chi_I(x)K(x,y)
\psi\biggl(
\frac{\Phi(x)-\Phi(y)}{\ell(I)}
...
e_{u,v,I}(x)h_{u,v,I}(y)
\end{align*}
for almost every \(x,y\in\Gamma\). Summing over
\(I\in\mathbb D_\Gamma\) and using
\eqref{eq:Weyl-off-diagonal-partition}, we obtain
\eqref{eq:Weyl-off-diagonal-NWO-expansion}.

This completes the proof of Lemma \ref{lem:Weyl-off-diagonal-kernel-NWO}.
\end{proof}

\begin{lemma}
\label{lem:Weyl-jump-OSC-coefficients}
Assume that \(N\geq 1\), \(1<p<\infty\), and \(1\leq q\leq\infty\). Fix \(s>2\), a closed Weyl chamber \(\Gamma\), and \(\sigma\in G\). Let \(r_s\) be as in Lemma \ref{lem:Dunkl-mixed-cube-to-Lebesgue-oscillation}. Then, for every \(f\in OSC^{p,q}(\mathbb R^N)\),
\begin{align}
&
\Bigg\|
\Bigg\{
\theta_{\sigma,\mathrm{Id},I}
\operatorname{osc}_{r_s}\bigl(H_{\sigma,\mathrm{Id}}(f);I\bigr)
\Bigg\}_{I\in\mathbb D_\Gamma}
\Bigg\|_{\ell^{p,q}}
+
\Bigg\|
\Bigg\{
\theta_{\sigma,\mathrm{Id},I}
a_{\sigma,\mathrm{Id},I}(f)
\Bigg\}_{I\in\mathbb D_\Gamma}
\Bigg\|_{\ell^{p,q}}
\lesssim
\|f\|_{OSC^{p,q}(\mathbb R^N)}.
\label{eq:Weyl-jump-OSC-coefficients}
\end{align}
The implicit constant is independent of \(f\) and \(\sigma\).
\end{lemma}

\begin{proof}
By \eqref{eq:D-and-theta}, \(0<\theta_{\sigma,\mathrm{Id},I}\leq 1\). Hence, by Lemma \ref{lem:Dunkl-H-OSC-Lorentz},
\begin{align*}
\Bigg\|
\Bigg\{
\theta_{\sigma,\mathrm{Id},I}
...
\|f\|_{OSC^{p,q}(\mathbb R^N)}.
\end{align*}
By Lemma \ref{lem:Dunkl-chamber-jump-supercritical0},
\begin{align*}
\Bigg\|
\Bigg\{
\theta_{\sigma,\mathrm{Id},I}
...
\|f\|_{OSC^{p,q}(\mathbb R^N)}.
\end{align*}
Combining the two estimates, we obtain \eqref{eq:Weyl-jump-OSC-coefficients}.

This completes the proof of Lemma \ref{lem:Weyl-jump-OSC-coefficients}.
\end{proof}

\begin{theorem}
\label{thm:Weyl-jump-multiplier-Schatten}
Assume that \(N\geq1\), \(1<p<\infty\), and \(1\leq q\leq\infty\). Fix a closed Weyl chamber \(\Gamma\) and \(\sigma\in G\setminus\{\mathrm{Id}\}\). Let \(T\) be an integral operator on \(L^2(\Gamma,dw)\) whose kernel \(K\) satisfies
\begin{align*}
\bigl|
\partial_x^\alpha\partial_y^\beta K(x,y)
\bigr|
...
\lesssim
\|f\|_{OSC^{p,q}(\mathbb R^N)}.
\label{eq:Weyl-jump-multiplier-Schatten}
\end{align}
\end{theorem}

\begin{proof}
Fix \(s>2\), let \(r_s\) be as in Lemma
\ref{lem:Dunkl-mixed-cube-to-Lebesgue-oscillation}, and set
\[
H:=H_{\sigma,\mathrm{Id}}(f)=f-f\circ\sigma.
\]
For \(I\in\mathbb D_\Gamma\), set
\[
\omega_I
:=
...
(H)_{I,dx}.
\]
By Lemma \ref{lem:Weyl-jump-OSC-coefficients}
\eqref{eq:Weyl-jump-OSC-coefficients},
\begin{align}
\Bigg\|
\Bigg\{
\theta_{\sigma,\mathrm{Id},I}\omega_I
\Bigg\}_{I\in\mathbb D_\Gamma}
\Bigg\|_{\ell^{p,q}}
+
\Bigg\|
\Bigg\{
\theta_{\sigma,\mathrm{Id},I}a_I
\Bigg\}_{I\in\mathbb D_\Gamma}
\Bigg\|_{\ell^{p,q}}
\lesssim
\|f\|_{OSC^{p,q}(\mathbb R^N)}.
\label{eq:Weyl-jump-two-coefficients}
\end{align}
In particular, \(\omega_I<\infty\) for every \(I\). By
Lemma \ref{lem:Dunkl-mixed-cube-to-Lebesgue-oscillation},
\(H\in L^s_{\mathrm{loc}}(\Gamma,dw)\).

Fix \(M_0>2N\). By Lemma
\ref{lem:Weyl-off-diagonal-kernel-NWO}, for almost every
\(x,y\in\Gamma\) with \(x\neq y\),
\begin{align}
K(x,y)
=
...
\theta_{\mathrm{Id},\sigma,I}
e_{u,v,I}(x)h_{u,v,I}(y),
\label{eq:Weyl-jump-use-kernel-expansion}
\end{align}
where
\[
\sum_{u,v\in\mathbb Z^N}|c_{u,v}|<\infty.
\]
By \eqref{eq:D-and-theta},
\(\theta_{\mathrm{Id},\sigma,I}
=\theta_{\sigma,\mathrm{Id},I}\).

For \(u,v\in\mathbb Z^N\) and \(I\in\mathbb D_\Gamma\), define
\begin{align*}
\widetilde e_{u,v,I}(x)
:=
\begin{cases}
\displaystyle
\omega_I^{-1}\bigl(H(x)-a_I\bigr)e_{u,v,I}(x),
&\omega_I>0,\\[2mm]
0,
&\omega_I=0.
\end{cases}
\end{align*}
By Lemma \ref{lem:Weyl-off-diagonal-kernel-NWO}
\eqref{eq:Weyl-off-diagonal-size} and Lemma
\ref{lem:Dunkl-mixed-cube-to-Lebesgue-oscillation},
\begin{align*}
\|\widetilde e_{u,v,I}\|_{L^s(dw)}
&\lesssim
w(I)^{-1/2}\omega_I^{-1}
\bigg(
\int_I|H-a_I|^s\,dw
...
w(I)^{\frac1s-\frac12}
\end{align*}
when \(\omega_I>0\), while the same estimate is immediate when
\(\omega_I=0\). Moreover, by Lemma
\ref{lem:Weyl-off-diagonal-kernel-NWO}
\eqref{eq:Weyl-off-diagonal-support}--\eqref{eq:Weyl-off-diagonal-size}
and \(w(I^*)\lesssim w(I)\),
\begin{align*}
\|e_{u,v,I}\|_{L^s(dw)}
+
...
Thus, for fixed \(u,v\),
\[
\bigl\{\widetilde e_{u,v,I}\bigr\}_{I\in\mathbb D_\Gamma},
\qquad
\bigl\{e_{u,v,I}\bigr\}_{I\in\mathbb D_\Gamma},
\qquad
\bigl\{\overline{h_{u,v,I}}\bigr\}_{I\in\mathbb D_\Gamma}
\]
are NWO families.

If \(\omega_I=0\), then \(H=a_I\) \(dx\)-a.e. on \(I\), and hence
\(dw\)-a.e. on \(I\). Therefore, in all cases,
\begin{align}
&\theta_{\sigma,\mathrm{Id},I}
H(x)e_{u,v,I}(x)h_{u,v,I}(y)
\nonumber\\
&\qquad=
\theta_{\sigma,\mathrm{Id},I}\omega_I
\widetilde e_{u,v,I}(x)h_{u,v,I}(y)
+
\theta_{\sigma,\mathrm{Id},I}a_I
e_{u,v,I}(x)h_{u,v,I}(y).
\label{eq:Weyl-jump-two-term-decomposition}
\end{align}
Hence, by Lemma \ref{NWOLEMMAA2} and
\eqref{eq:Weyl-jump-two-coefficients},
\begin{align}
&\Bigg\|
\sum_{I\in\mathbb D_\Gamma}
\theta_{\sigma,\mathrm{Id},I}\omega_I
\bigl\langle
\,\cdot,\overline{h_{u,v,I}}
\bigr\rangle
\widetilde e_{u,v,I}
+
\sum_{I\in\mathbb D_\Gamma}
...
&\qquad\lesssim
\|f\|_{OSC^{p,q}(\mathbb R^N)},
\label{eq:Weyl-jump-fixed-Fourier-Schatten}
\end{align}
uniformly in \(u,v\).

Since \(p>1\), using an equivalent norm on \(S^{p,q}\) when necessary, the series
\begin{align}
S_f
:=
\sum_{u,v\in\mathbb Z^N}
...
e_{u,v,I}
\Bigg)
\label{eq:Weyl-jump-Sf}
\end{align}
converges in \(S^{p,q}(L^2(\Gamma,dw))\). By
\eqref{eq:Weyl-jump-fixed-Fourier-Schatten},
\begin{align}
\|S_f\|_{S^{p,q}}
\lesssim
\sum_{u,v\in\mathbb Z^N}
...
\lesssim
\|f\|_{OSC^{p,q}(\mathbb R^N)}.
\label{eq:Weyl-jump-Sf-bound}
\end{align}

We identify \(S_f\) with \(M_HT\). Fix \(r>p\). By
\(\ell^{p,q}\subset\ell^r\) and
\eqref{eq:Weyl-jump-two-coefficients},
\begin{align*}
\Bigg\{
\theta_{\sigma,\mathrm{Id},I}\omega_I
...
\in\ell^r.
\end{align*}
For \(m\in\mathbb N\), let \(S_f^{(m)}\) be defined by
\eqref{eq:Weyl-jump-Sf} with the sum over \(I\) restricted to
\(\bigcup_{|k|\leq m}\mathbb D_{\Gamma,k}\). By Lemma
\ref{NWOLEMMAA2}, now with exponent \(r\),
\begin{align*}
\|S_f-S_f^{(m)}\|_{S^r}
\lesssim
\sum_{u,v\in\mathbb Z^N}|c_{u,v}|
\Bigg(
&
\Bigg\|
\Bigg\{
...
Let
\(\varphi\in C_c^\infty(\Gamma^\circ)\) and
\[
\eta\in
C_c^\infty
\bigl(
\Gamma^\circ\setminus\{x:\sigma x=x\}
\bigr).
\]
By the construction in the proof of Lemma
\ref{lem:Weyl-off-diagonal-kernel-NWO} and
\eqref{eq:Weyl-jump-two-term-decomposition},
\begin{align}
\bigl\langle S_f^{(m)}\varphi,\eta\bigr\rangle
=
...
\nonumber\\
&\qquad\qquad\times
\varphi(y)\overline{\eta(x)}
\,dw(y)\,dw(x).
\label{eq:Weyl-jump-truncated-kernel}
\end{align}
...
\delta
:=
\inf_{x\in\operatorname{supp}\eta}
\|x-\sigma x\|>0.
\]
For \(x\in\operatorname{supp}\eta\) and
\(\|x-y\|<\delta/2\),
\begin{align*}
\|x-\sigma y\|
\geq
...
\frac{\delta}{2}.
\end{align*}
Moreover, since \(x,y\in\Gamma\), by \eqref{distanceequa} and the
\(G\)-invariance of \(d\),
\begin{align*}
\|x-\sigma y\|
\geq
...
\|x-y\|.
\end{align*}
Consequently, the kernel estimate and the doubling property imply
\begin{align*}
\int_{\substack{y\in\Gamma\\ \|x-y\|<\delta/2}}
|K(x,y)|\,dw(y)
&\lesssim
\sum_{j=1}^{\infty}
\frac{2^{-j}\delta}{\delta}
\frac{
w\bigl(B(x,2^{-j}\delta)\bigr)
}{
w\bigl(B(x,2^{-j-1}\delta)\bigr)
}
\lesssim
\sum_{j=1}^{\infty}2^{-j}
\lesssim1
\end{align*}
uniformly in \(x\in\operatorname{supp}\eta\). On
\(\operatorname{supp}\eta\times\operatorname{supp}\varphi\) with
\(\|x-y\|\geq\delta/2\), the same annular argument involves only
finitely many scales. Hence,
\begin{align}
\sup_{x\in\operatorname{supp}\eta}
\int_{\operatorname{supp}\varphi}
|K(x,y)|\,dw(y)
<\infty.
\label{eq:Weyl-jump-local-kernel-integrability}
\end{align}
Since \(H\in L^s_{\mathrm{loc}}(\Gamma,dw)\) with \(s>2\),
\(H\eta\in L^2(\Gamma,dw)\cap L^1(\Gamma,dw)\). The partial sums in
\eqref{eq:Weyl-jump-truncated-kernel} are uniformly bounded and
converge to \(1\) for \(x\neq y\). Therefore, by
\eqref{eq:Weyl-jump-local-kernel-integrability} and dominated
convergence,
\begin{align*}
\lim_{m\to\infty}
\bigl\langle S_f^{(m)}\varphi,\eta\bigr\rangle
...
\bigr\rangle.
\end{align*}
On the other hand, the convergence in \(S^r\) implies
\[
\lim_{m\to\infty}
\bigl\langle S_f^{(m)}\varphi,\eta\bigr\rangle
=
\bigl\langle S_f\varphi,\eta\bigr\rangle.
\]
Since \(\sigma\neq\mathrm{Id}\), the fixed point space
\(\{x:\sigma x=x\}\) is a proper linear subspace and therefore has
\(dw\)-measure zero. Thus,
\(C_c^\infty\bigl(\Gamma^\circ\setminus\{x:\sigma x=x\}\bigr)\)
is dense in \(L^2(\Gamma,dw)\), and \(S_f\) is the bounded extension
of \(M_HT=M_{f-f\circ\sigma}T\). Combining this identification with
\eqref{eq:Weyl-jump-Sf-bound}, we obtain
\eqref{eq:Weyl-jump-multiplier-Schatten}.

This completes the proof of Theorem \ref{thm:Weyl-jump-multiplier-Schatten}.
\end{proof}
%
%\begin{proof}
